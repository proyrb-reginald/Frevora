 ...
 * @note   The RCC_CFG0, RCC_CFG1, and PLL_CFG write operations require the
 * RCC_KEY to be unlocked.
 * @param  TimeLimit[in] : Switching time limit
 *          This parameter needs to be greater than or equal to 0x40.
 * @note   If no register write command is received after a timeLimit system
 * clock, the RCC rewrite function is disabled again.
 * @retval Set state:
 *               - SUCCESS
 *               - ERROR
 */
                   ...
                   RLL_Factor_TypeDef*      RLL_Factor)
{
    ...
    tmpreg &= (uint32_t)~(PLL_CFG_PLLCLKSEL | PLL_CFG_MDIVM | PLL_CFG_NDIVN |
                          PLL_CFG_PDIVP);

    tmpreg |=
        (uint32_t)(RCC_PLLCLKSource | (RLL_Factor->PLLM << PLL_CFG_MDIVM_Pos) |
 ...
 * @param  RCC_Clocks[out]: Pointer to structure RCC_ClocksTypeDef, to be
 * initialized.
 *
 * @retval None
 */
 ...
 * @param  RCC_APB0Periph[in]: specifies the APB01 peripheral to gates its
 * clock. SC32f10xx Selection
 ...
 *             SC32f11xx Selection
 * range(RCC_APB1Periph_TIM4-RCC_APB1Periph_TIM7,RCC_APB1Periph_TWI1,RCC_APB1Periph_UART2,RCC_APB1Periph_UART4,RCC_APB1Periph_ALL)
 *             SC32f12xx Selection
 ...
 * @param  RCC_APB2Periph[in]: specifies the APB21 peripheral to gates its
 * clock. SC32f10xx Selection
 ...
 * range(RCC_NMIPeriph_CSS,RCC_NMIPeriph_CMP,RCC_NMIPeriph_INT0,RCC_NMIPeriph_ALL,RCC_NMIPeriph_SRAMPE)
 *             SC32f15xx Selection
 * range(RCC_NMIPeriph_INT0,RCC_NMIPeriph_SRAMPE,RCC_NMIPeriph_OP2,RCC_NMIPeriph_OP1,RCC_NMIPeriph_CMP��RCC_NMIPeriph_MCMP0,RCC_NMIPeriph_ALL)
...
void RCC_SystickCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        SysTick->CTRL |=
            (uint32_t)(SysTick_CTRL_ENABLE_Msk |
                       SysTick_CTRL_TICKINT_Msk); /* Enable SysTick IRQ and
                                                     SysTick Timer */
    }
    else {
        SysTick->CTRL &=
            ~(uint32_t)(SysTick_CTRL_ENABLE_Msk | SysTick_CTRL_TICKINT_Msk);
    }
}
/**
 ...
 *                         SC32f10xx Selection
 * range(RCC_FLAG_CLKIF,RCC_FLAG_LOCKERR,RCC_FLAG_PLLRDY) SC32f12xx Selection
 * range(RCC_FLAG_CLKIF,RCC_FLAG_SRAMPEIF)
