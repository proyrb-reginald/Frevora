...
void CAN_Init(CAN_TypeDef* CANx, CAN_InitTypeDef* CAN_InitStruct)
{
    ...
    if (CAN_InitStruct->CAN_PTB_AutoRetrans == ENABLE) {
        CANx->CAN_CFG_STAT |= CAN_TPSS_Enable;
    }
    else {
    ...
    if (CAN_InitStruct->CAN_STB_AutoRetrans == ENABLE) {
        CANx->CAN_CFG_STAT |= CAN_TSSS_Enable;
    }
    else {
    ...
    CANx->CAN_ACFCTRL |= CAN_InitStruct->CANTimeStampPositon
                         << CAN_ACFCTRL_TIMEPOS_Pos;
    ...
    CANx->CAN_CFG_STAT |= CAN_InitStruct->CANTBUFSelect |
                          CAN_InitStruct->CANFDISOSelect |
                          CAN_InitStruct->CANTSMODESelect;
}

/**
 * @brief  Fills each CAN_InitStruct member with its default value.
 * @param  CAN_InitStruct[out]: Pointer to structure CAN_InitStruct, to be
 * initialized.
 * @retval None.
 */
...
void CAN_TBUFSelect(CAN_TypeDef* CANx, CANTBUF_TypeDef CANTBUF)
{
    if (CANTBUF == CAN_TBUFTx_STB)
        CAN->CAN_CFG_STAT |= CAN_TBUFTx_STB;  // Send buffer Select STB
    if (CANTBUF == CAN_TBUFTx_PTB)
        CAN->CAN_CFG_STAT &= ~CAN_TBUFTx_STB;  // Send buffer Select PTB
}
/**
 ...
 *                    - CAN_Baudrate_125kHz:Select set the high speed baud rate
 * of CAN to 125khz
 *                    - CAN_Baudrate_250kHz:Select set the high speed baud rate
 * of CAN to 250khz
 *                    - CAN_Baudrate_500kHz:Select set the high speed baud rate
 * of CAN to 500khz
 *                    - CAN_Baudrate_800kHz:Select set the high speed baud rate
 * of CAN to 800khz
 *                    - CAN_Baudrate_1000kHz:Select set the high speed baud rate
 * of CAN to 1000khz
 * @retval None.
 */
                          ...
                          uint32_t     BaudRate)
{
    ...
    CAN->CAN_F_SEG |= (F_PRESCALER << CAN_F_PRESC_Pos) |
                      (F_SJW << CAN_F_SJW_Pos) | (F_SEG2 << CAN_F_SEG_2_Pos) |
                      F_SEG1;
}
                          ...
                          uint32_t     BaudRate)
{
    ...
    CAN->CAN_S_SEG |= (S_PRESCALER << CAN_S_PRESC_Pos) |
                      (S_SJW << CAN_S_SJW_Pos) | (S_SEG2 << CAN_S_SEG_2_Pos) |
                      S_SEG1;
}
/**
...
void CAN_FliterInit(CAN_FliterTypeDef* CAN_FliterStruct)
{
    ...
    switch (CAN_FliterStruct->CANFliterFrame) {
        ...
        case CAN_Fliter_Default: CAN->CAN_ACF &= ~(CAN_ACF_AIDEE); break;
 ...
 * @param  CANx[out]:where x can be select the CANx peripheral.
 *                  - CAN:select the CAN peripheral.
 * @param  CAN_Tx_msg[out]: Pointer to structure CAN_Tx_msg_Typedef, to be
 * initialized.
 * @retval None.
 */
void CAN_Trans_Init(CAN_TypeDef* CANx, CanTxMsg* CAN_Tx_msg)
{
    ...
    /*Since the values in BUF are garbled after each burn, they need to be
     * cleared each time slot is replaced*/
...
void CAN_TxDCompensateCmd(CAN_TypeDef* CANx, FunctionalState NewState)
{
    ...
    CAN->CAN_CFG_STAT &= ~0x80;
    CAN->CAN_CFG_STAT = temp;
}
/**
 * @brief  CAN peripherals send initialization.
 * @param  CANx[out]:where x can be select the CANx peripheral.
 *                  - CAN:select the CAN peripheral.
 * @param  RxMessage[out]:[out]: Pointer to structure CAN_RxMsg_Typedef, to be
 * initialized.
 * @retval None.
 */
 ...
 *                 - CAN_RAFIEEN:Select the RB slots Threshold warning Indicates
 * the interrupt.
 *                 - CAN_TPIEEN:Select the interruption of PTB transmission
 * completion.
 *                 - CAN_TSIEEN:Select the STB transmission completion
 * interrupt.
 ...
 *                  -  CAN_RIF_FlAG:Select the RIF_FlAG
 *                  -  CAN_TSFF_FLAG:Select the TSFF_FLAG.
 *                  - 	CAN_AIF_FLAG:Select the AIF_FLAG.
 *                  - 	CAN_EIF_FLAG:Select the EIF_FLAG.
 *                  - 	CAN_TSIF_FLAG:Select the TSIF_FLAG.
 *                  - 	CAN_TPIF_FLAG:Select the TPIF_FLAG.
 *                  - 	CAN_RAFIF_FLAG:Select the RAFIF_FLAG.
 *                  - 	CAN_RFIF_FLAG:Select the RFIF_FLAG.
 *                  - 	CAN_ROIF_FLAG:Select the ROIF_FLAG.
 *                  - 	CAN_ALIF_FLAG:Select the ALIF_FLAG.
 *                  - 	CAN_BEIF_FLAG:Select the BEIF_FLAG.
 *                  - 	CAN_EPIF_FLAG:Select the EPIF_FLAG.
 ...
 * @param  CANx[out]:where x can be select the CANx peripheral.
 *                  - CAN:select the CAN peripheral.
 * @param  Errorcnt:Set CANx error threshold
...
/* End of CAN_Group4.	*/
/** @defgroup CAN_Group5Timestamp function implementation configuration
 ...
 * @param  CANx[out]:where x can be select the CANx peripheral.
 *                  - CAN:select the CAN peripheral.
 * @param  CANTimeStampPosition:The location of the timestamp
 *                  -CAN_TimeStampPosition_SOF:Select the frame start as the
 * timestamp location -CAN_TimeStampPosition_EOF:Select the end of frame as the
 * timestamp location
 */
