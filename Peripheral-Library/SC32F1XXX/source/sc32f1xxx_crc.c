...
/**
 * @brief  DeInitialize the CRC peripheral registers to their default reset
 * values.
 * @retval None
 */
...
/**
 * @brief  Initializes the peripheral CRC register according to the parameters
 * specified in CRC_InitStruct
 * @param  CRC_InitStruct[out]:Pointer to structure CRC_InitTypeDef, to be
 * initialized.
 * @retval None
 */
 ...
 *                       - CRC_InputData_Format_HALFWORDS:Input data in
 * half-word format
 *                       - CRC_InputData_Format_WORDS:Input data in word format
 * @param  pBuffer[in]: pointer to the input data buffer, exact input data
                        ...
                        uint32_t                     BufferLength)
{
    ...
    switch (InputDataFormat) {
        ...
        case CRC_InputData_Format_HALFWORDS:
            temp =
                CRC_Handle_16((uint16_t*)(void*)pBuffer,
 ...
 * half-word format
 *                       - CRC_InputData_Format_WORDS:Input data in word format
 * @param  pBuffer[in]:pointer to the input data buffer, exact input data format
 * is provided by hcrc->InputDataFormat.
 * @param  BufferLength[in]:Input data buffer length (number of bytes if pBuffer
 *         type is * uint8_t, number of half-words if pBuffer type is *
 * uint16_t, number of words if pBuffer type is * uint32_t).
 * @retval uint32_t CRC (returned value LSBs for CRC shorter than 32 bits)
...
static uint32_t CRC_Handle_8(uint8_t pBuffer[], uint32_t BufferLength)
{
    ...
    /* Processing time optimization: 4 bytes are entered in a row with a single
     * word write, last bytes must be carefully fed to the CRC calculator to
    ...
    if ((BufferLength % 4U) != 0U) {
        ...
        if ((BufferLength % 4U) == 2U) {
            ...
            pReg = (__IO uint16_t*)(__IO void*)(&CRC->CRC_DR); /* Derogation
                                                                  MisraC2012
                                                                  R.11.5 */
            *pReg = data;
        }
...
static uint32_t CRC_Handle_16(uint16_t pBuffer[], uint32_t BufferLength)
{
     ...
     * single word write, in case of odd length, last HalfWord must be carefully
     * fed to the CRC calculator to ensure a correct type handling by the
     * peripheral */
    for (i = 0U; i < (BufferLength / 2U); i++) {
        CRC->CRC_DR = ((uint32_t)pBuffer[2U * i] << 16U) |
                      (uint32_t)pBuffer[(2U * i) + 1U];
    }
    if ((BufferLength % 2U) != 0U) {
        pReg =
            (__IO uint16_t*)(__IO void*)(&CRC->CRC_DR); /* Derogation MisraC2012
                                                           R.11.5 */
        *pReg = pBuffer[2U * i];
    }
