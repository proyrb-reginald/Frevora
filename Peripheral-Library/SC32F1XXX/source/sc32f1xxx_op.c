...
/** @defgroup OP_Exported_Functions_Group1 Configuration of the OP computation
unit functions
...
/**
 * @brief  DeInitialize the OP peripheral registers to their default reset
 * values.
 ...
 * @param  OP_InitStruct[out]:Pointer to structure OP_InitTypeDef, to be
 * initialized.
 * @retval None
 */
...
#    if defined(SC32f12xx)
    ...
    /* Set OPFS bit to OP_FreqSelect value */
    tmpreg |=
        (uint32_t)(OP_InitStruct->OP_FDBResisrance |
                   OP_InitStruct->OP_Negative | OP_InitStruct->OP_Output |
                   OP_InitStruct->OP_PGAGain | OP_InitStruct->OP_Posittive |
                   OP_InitStruct->OP_ShortCircuit);
...
#    elif defined(SC32f15xx)
    ...
    tmpreg |=
        (uint32_t)(OP_InitStruct->OP_OutputPin | OP_InitStruct->OP_InvertInput |
                   OP_InitStruct->OP_NonInvertInput |
                   OP_InitStruct->OP_PGAGain | OP_InitStruct->OP_OPRF |
                   OP_InitStruct->OP_FDBR);
 ...
 * @param  OP_InitStruet[out]: pointer to an OP_InitTypeDef structure that
...
#    if defined(SC32f12xx)
    ...
    tmpreg |= (uint32_t)((OP_TrimValueL << OP_CON_TRIMOFFSETNL_Pos) |
                         (OP_TrimValueH << OP_CON_TRIMOFFSETNH_Pos));
 ...
 *                  - OP_PGAGain_NonInvert4_Invert3:The OP inverting input gain
 * is 4/3 times
 *                  - OP_PGAGain_NonInvert8_Invert7:The OP inverting input gain
 * is 8/7 times
 *                  - OP_PGAGain_NonInvert16_Invert15:The OP inverting input
 * gain is 16/15 times
 *                  - OP_PGAGain_NonInvert32_Invert31:The OP in-phase input gain
 * is 32/31 times
 *                  - OP_PGAGain_NonInvert64_Invert63:The OP in-phase input gain
 * is 64/63 times
 * @retval None
 */
 ...
 *               - OP_2: select the OP2 peripheral
 * @param  OPNonInvertInput[in]: No invert input port selection.
 *               - OP_OPRF_1D16OP_VREF:Opamp inverter input voltage select
 * 1D16OP_VREF
 *               - OP_OPRF_2D16OP_VREF:Opamp inverter input voltage select
 * 2D16OP_VREF
 *               - OP_OPRF_3D16OP_VREF:Opamp inverter input voltage select
 * 3D16OP_VREF
 *               - OP_OPRF_4D16OP_VREF:Opamp inverter input voltage select
 * 4D16OP_VREF
 *               - OP_OPRF_5D16OP_VREF:Opamp inverter input voltage select
 * 5D16OP_VREF
 *               - OP_OPRF_6D16OP_VREF:Opamp inverter input voltage select
 * 6D16OP_VREF
 *               - OP_OPRF_7D16OP_VREF:Opamp inverter input voltage select
 * 7D16OP_VREF
 *               - OP_OPRF_8D16OP_VREF:Opamp inverter input voltage select
 * 8D16OP_VREF
 *               - OP_OPRF_9D16OP_VREF:Opamp inverter input voltage select
 * 9D16OP_VREF
 *               - OP_OPRF_10D16OP_VREF:Opamp inverter input voltage select
 * 10D16OP_VREF
 *               - OP_OPRF_11D16OP_VREF:Opamp inverter input voltage select
 * 11D16OP_VREF
 *               - OP_OPRF_12D16OP_VREF:Opamp inverter input voltage select
 * 12D16OP_VREF
 *               - OP_OPRF_13D16OP_VREF:Opamp inverter input voltage select
 * 13D16OP_VREF
 *               - OP_OPRF_14D16OP_VREF:Opamp inverter input voltage select
 * 14D16OP_VREF
 *               - OP_OPRF_15D16OP_VREF:Opamp inverter input voltage select
 * 15D16OP_VREF
 * @retval None
 */
...
void OP_InputSelection(OP_TypeDef* OPx, OP_InvertInput_TypeDef OPInvertInput)
{
    ...
    assert_param(IS_OP_ALL_PERIPH(OPx));
    if (OPx == OP_0) {
        if (OPInvertInput == OP_InvertInput_OPxN ||
            OPInvertInput == OP_InvertInput_Res) {
 ...
 *               -OP_TriggerMode_RISE:The trigger mode of the simulated
 * comparator is rising edge -OP_TriggerMode_FALL:The trigger mode of simulated
 * comparator is falling edge -OP_TriggerMode_RISE_FALL:The trigger mode of the
 * simulated comparator is rising edge and falling edge
 * @retval None
 */
...
void OP_OffsetZero(OP_TypeDef* OPx)
{
    ...
    if (OPx == OP_0) {
        ...
        /*!< Configure the new trimming value */
        tmpreg |= (uint32_t)((OP_Nmos_offset << OP_CON_TRIMOFFSETP_Pos) |
                             (OP_Pmos_offset << OP_CON_TRIMOFFSETN_Pos));
 ...
 *               - OP_CMPSTA_Low: The non-inverting input is at a lower voltage
 * than the inverting input
 *               - OP_CMPSTA_High: The non-inverting input is at a higher
 ...
 * @param  OP_IT[in]: specifies the OP interrupts sources to be enabled or
 * disabled.
