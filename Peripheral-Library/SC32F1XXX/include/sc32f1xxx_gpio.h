...
#define IS_GPIO_BITACTION(STATE)                                               \
    (((STATE) == Bit_RESET) || ((STATE) == Bit_SET))
...
typedef struct
{
    ...
    uint16_t GPIO_Mode; /*!< Specifies the operating mode for the selected pins.
                           ...
                           GPIO_Mode_TypeDef   */
    uint16_t GPIO_DriveLevel; /*!< Specifies the operating Drive Level for the
                                 selected pins. This parameter can be a value of
                                 @ref GPIO_DriveLevel_TypeDef   */
} GPIO_InitTypeDef;
