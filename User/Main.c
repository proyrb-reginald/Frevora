...
void vTask_Monitor(void *pvParameters)
{
	...
	for (;;)
	{
		...
		Terminal_Output("[%s] Free Memory:\n Heap: %u Bytes\n %s: %u Words\n",
			Task_Name, xPortGetMinimumEverFreeHeapSize(), pcTaskGetName(vTask_List[0]),
...
{
    SC_Init(); // 初始化芯片外设
	DMA_Buffer_Manager_Initialize(&DMA_Buffer_Manager_UART1, 256, DMA1, UART1, DMA_UART); // 初始化DMA缓冲区管理器
