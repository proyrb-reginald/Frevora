...
static void DMA_Buffer_Manager_Start(DMA_Buffer_Manager* const Manager)
{
    ...
    if (Manager->_Head > Manager->_Tail) {
        ...
        // 环绕情况：数据从缓冲区末尾绕回开头
        Manager->_Transmitting_Length =
            Manager->_Buffer_Length - Manager->_Tail;
    }
    // 配置DMA寄存器
    DMA_SetSrcAddress(
        Manager->_Select_DMA,
        (uint32_t)&(Manager->_Buffer[Manager->_Tail]));  // 设置源地址
    DMA_SetCurrDataCounter(Manager->_Select_DMA,
                           Manager->_Transmitting_Length);  // 设置数据量
...
{
    // 参数有效性检查
    if ((Manager == NULL) ||                               // 管理器实例非空
        (Buffer_Length == 0) ||                            // 缓冲区长度非零
        ((Buffer_Length & (Buffer_Length - 1)) != 0) ||    // 必须为2的幂次方
        ((Select_DMA != DMA0) && (Select_DMA != DMA1)) ||  // DMA控制器有效性
        ((Select_Peripheral != UART0) && (Select_Peripheral != UART1) &&
         (Select_Peripheral != SPI0) && (Select_Peripheral != SPI1) &&
         (Select_Peripheral != TWI0)) ||  // 外设地址有效性
        ((Peripheral_Type != DMA_UART) && (Peripheral_Type != DMA_SPI) &&
         (Peripheral_Type != DMA_TWI))  // 外设类型有效性
    ) {
        while (1)
            ;  // 参数错误进入死循环（需根据实际项目替换为错误处理）
    }
    // 缓冲区初始化/重配置
    if (Manager->_Buffer == NULL) {
        ...
        if (Buffer_Length != Manager->_Buffer_Length) {
            ...
            Manager->_Buffer = (volatile uint8_t*)pvPortMalloc(
                Buffer_Length);              // 重新分配指定大小的内存
            configASSERT(Manager->_Buffer);  // 内存分配检查
            Manager->_Buffer_Length = Buffer_Length;
        }
    }
    ...
    switch (Peripheral_Type) {
        case DMA_UART:
            DMA_InitStruct.DMA_DstAddress =
                (uint32_t)&((UART_TypeDef*)Select_Peripheral)->UART_DATA;
        ...
        case DMA_SPI:
            DMA_InitStruct.DMA_DstAddress =
                (uint32_t)&((SPI_TypeDef*)Select_Peripheral)->SPI_DATA;
            switch ((uint32_t)Select_Peripheral) {
                case (uint32_t)SPI0:
                    DMA_InitStruct.DMA_Request = DMA_Request_SPI0_TX;
                    break;
                case (uint32_t)SPI1:
                    DMA_InitStruct.DMA_Request = DMA_Request_SPI1_TX;
                    break;
                default:
                    while (1)
                        ;  // 未知UART外设，进入死循环或错误处理
        ...
        case DMA_TWI:
            DMA_InitStruct.DMA_DstAddress =
                (uint32_t)&((TWI_TypeDef*)Select_Peripheral)->TWI_DATA;
            switch ((uint32_t)Select_Peripheral) {
                case (uint32_t)TWI0:
                    DMA_InitStruct.DMA_Request = DMA_Request_TWI0_TX;
                    break;
                default:
                    while (1)
                        ;  // 未知UART外设，进入死循环或错误处理
                                  ...
                                  uint16_t                  Data_Input_Length)
{
    ...
    if (xSemaphoreTake(Manager->_Resource_Occupy, portMAX_DELAY) == pdTRUE) {
        ...
        uint16_t Free_Buffer_Length =
            (Manager->_Buffer_Length - (Manager->_Head - Manager->_Tail + 1)) &
            (Manager->_Buffer_Length -
             1);  // 计算可用空间（考虑环形缓冲区特性）
                  // 缓冲区满
        ...
        // 第一段拷贝
        memcpy((void*)&(Manager->_Buffer[Manager->_Head]), Data_Pointer,
               First_Input_Length);
        Manager->_Head = (Manager->_Head + First_Input_Length) &
                         (Manager->_Buffer_Length - 1);
        uint16_t Inputted = First_Input_Length;
        // 第二段拷贝（如果存在环绕）
        if (Data_Input_Length > First_Input_Length) {
            uint16_t Second_Input_Length =
                Data_Input_Length - First_Input_Length;
            memcpy((void*)&Manager->_Buffer[Manager->_Head + 1],
                   &Data_Pointer[First_Input_Length], Second_Input_Length);
            Manager->_Head = (Manager->_Head + Second_Input_Length + 1) &
                             (Manager->_Buffer_Length - 1);
...
void DMA_Buffer_Manager_IRQHandler(DMA_Buffer_Manager* const Manager)
{
    ...
    Manager->_Tail = (Manager->_Tail + Manager->_Transmitting_Length) &
                     (Manager->_Buffer_Length - 1);
    // 如果仍有待传输数据，启动下一次传输
    if (Manager->_Head != Manager->_Tail) {
        DMA_Buffer_Manager_Start(Manager);
